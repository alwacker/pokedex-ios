...
struct PokemonPageResult: Decodable {
    let name: String
    let url: String
}
...
struct Pokemon: Decodable {
    ...
    let sprites: Sprites?
    let weight: Int
    let types: [PokemonTypeListObject]

    var heightText: String {
        return "\((Float(height ?? 1) / 10)) cm"
    }

    var weightText: String {
        return "\(Float(weight / 10)) kg"
    }
    ...
    var imageURL: URL? {
        let urlString = sprites?.other?.officialArtwork?.frontDefault ?? ""
        return URL(string: urlString)
    }
}

struct PokemonTypeListObject: Decodable {
    let slot: Int
...
enum PokemonKind: String, Decodable {
    case normal
    case fighting
    case flying
    case poison
